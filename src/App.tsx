/* ============================================================================
   V8 CRM — Design System showcase.
   A living style guide: it documents the tokens and shows them applied to real
   CRM primitives. Deliberately NOT a copy of the reference layout — the type
   scale, spacing rhythm, and motion are borrowed; the arrangement is ours.
   ========================================================================== */
...
export function App() {
  ...
  return (
    <div className="mx-auto max-w-[1120px] px-6 py-10">
      {/* ---- Masthead ---- */}
      <header className="flex flex-wrap items-center justify-between gap-4 animate-fade-rise">
        <div className="flex items-center gap-3">
          <BrandMark size={40} />
          <div>
            <div className="flex items-center gap-2">
              <span className="text-h3 font-semibold">V8 CRM</span>
              <Badge tone="accent" dot={false}>
                Design System
              </Badge>
            </div>
            <p className="text-body-sm text-text-muted">
              Tokens, primitives & motion for the V8 operating console
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="ghost">Read the tokens</Button>
          <Button variant="primary">Use in a screen</Button>
        </div>
      </header>

      <hr className="my-8 border-[color:var(--v8-border)]" />
      ...
      <div className="flex flex-col gap-16">
        ...
        <Section
          n="01"
          title="Metrics"
          desc="The KPI tile and hero chart — the reference's readouts, rebuilt on V8 tokens."
        >
          <div className="mb-4 flex items-center justify-between">
            <span className="eyebrow">This quarter</span>
            <SegmentedControl
              options={["1D", "1W", "1M", "1Q", "1Y"]}
              value={range}
              onChange={setRange}
            />
          </div>
          ...
          <StatCardRow>
            <StatCard
              ...
              trend={8.27}
              range={range}
              series={[28, 30, 29, 33, 36, 34, 39, 42]}
              delay={0}
            />
            <StatCard
              ...
              trend={4.12}
              range={range}
              series={[10, 12, 11, 14, 13, 16, 18, 21]}
              delay={60}
            />
            <StatCard
              label="Win Rate"
              ...
              trend={-1.42}
              range={range}
              series={[18, 17, 17, 15, 16, 14, 13, 13]}
              delay={120}
            />
            <StatCard
              ...
              trend={2.4}
              range={range}
              series={[5, 5, 6, 6, 7, 7, 8, 9]}
              delay={180}
            />
          </StatCardRow>

          {/* Hero chart panel — demonstrates the large chart surface & tooltip. */}
        ...
        <Section
          ...
          desc="A tight, deliberate ramp. Display sizes carry the numeric readouts; body and below hold the same rhythm as the reference."
        >
        ...
        <Section
          n="03"
          title="Color"
          desc="Near-black surfaces in elevation layers, a single violet signal, and status tints reserved for meaning — never decoration."
        >
        ...
        <Section
          ...
          desc="One unit is 4px. Layouts compose from this ladder only, so horizontal and vertical rhythm stay locked."
        >
        ...
        <Section
          ...
          desc="Short, calm, ease-out. Hover and press are fast feedback; toggles and entrances run one step longer. Hover the controls to feel the timing."
        >
        ...
        <Section
          n="06"
          title="In context"
          desc="The primitives assembled into a real CRM surface — the accounts table V8 uses to run its book of engagements."
        >
          <AccountsTable accounts={accounts} />
        </Section>
      </div>

      <footer className="mt-16 flex items-center justify-between border-t border-[color:var(--v8-border)] pt-6 text-body-sm text-text-muted">
        <span>V8 CRM · Design System v0.1</span>
        <span className="tabular">Tokens → Tailwind → primitives</span>
      </footer>
    </div>
  );
}
